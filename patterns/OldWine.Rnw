...
\Problem
  ...
  Intent: Discard bug reports that are too recent to be classified.
  
  How can we deal with recent, incomplete information in bug reports?
...
\Context
  ...
  Bug reports change over time. Sometimes, one needs to classify a bug report according to the eventual occurence of some change. For example, has a bug been fixed (or will it, eventually)? Is this a bug that was or will be reopened, or it will never be reopened? 

  While for older bugs such questions can be answered with high confidence, for bugs that are too recent, the change we are interested may not have happened---\emph{yet}. The question is how long to wait for the change to happen before assuming that it will never happen for a particular bug report.
...
@
  ...
  For the sake of example, consider we are studying bug reopenings. If the data shows that a bug report was reopened some day, classify it as a reopening bug. If not, then consider its lifetime, from creation to the last date available in the data. If the lifetime is long enough (i.e., it is above some \emph{threshold}), then classify it as non-reopening, which means that the bug was not and, with high confidence, will never be reopened. If the lifetime is short, however, it is hard to predict whether the bug report will eventually reopen, hence discard it from the analysis.
