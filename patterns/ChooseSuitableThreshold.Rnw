...
\Discussion
  ...
  Sometimes, thresholds can be inferred from natural human limits. For example, how many bugs a developer can fix in a single day? A reasonable answer can be used as a threshold to filter out invalid data.
