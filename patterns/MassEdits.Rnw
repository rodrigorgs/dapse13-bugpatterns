...
\Example
  ...
  For example, Souza and Chavez~\cite{Souza2012} used the first solution (see Figure 2 in their paper) and a variation of the second without taking comments into account to detect mass verifications (i.e., changes which set the status to {\tt VERIFIED}). They discarded all changes that were part of mass updates which changed at least 50 bug reports.
