...
@
  ...
  \emph{Solution 2: count the number of changes per user per day}. Then, look for high counts. As always, choose a threshold for the number of changes, so that higher numbers are evidence of mass edits. Assume that all changes made by the developer that day are the result of a mass edit.
